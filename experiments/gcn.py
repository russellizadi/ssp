...
parser.add_argument('--runs', type=int, default=5) # 100
parser.add_argument('--epochs', type=int, default=200)
parser.add_argument('--lr', type=float, default=0.01)
parser.add_argument('--weight_decay', type=float, default=0.0005)
parser.add_argument('--early_stopping', type=int, default=0) #10
...
if args.hyperparam == 'eps':
    ...
    for param in np.linspace(1., 10., 10, endpoint=True):
        ...
        kwargs[args.hyperparam] = param
        run(**kwargs)
elif args.hyperparam == 'alpha':
    for param in np.linspace(1., 0.9, 10, endpoint=True):
        print(f"{args.hyperparam}: {param}")
        kwargs[args.hyperparam] = param
        run(**kwargs)
else:
